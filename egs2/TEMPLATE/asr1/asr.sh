...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ] && ! [[ " ${skip_stages} " =~ [[:space:]]6[[:space:]] ]]; then
    log "Stage 6: LM collect stats: train_set=${data_feats}/lm_train_filtered_2000.txt, dev_set=${lm_dev_text}"
    ...
    _nj=$(min "${nj}" "$(<${data_feats}/lm_train_filtered_2000.txt wc -l)" "$(<${lm_dev_text} wc -l)")

    key_file="${data_feats}/lm_train_filtered_2000.txt"
    split_scps=""
    for n in $(seq ${_nj}); do
    ...
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        ${python} -m espnet2.bin.lm_train \
            ...
            --cleaner "${cleaner}" \
            --g2p "${g2p}" \
            --train_data_path_and_name_and_type "${data_feats}/lm_train_filtered_2000.txt,text,text" \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ] && ! [[ " ${skip_stages} " =~ [[:space:]]7[[:space:]] ]]; then
    log "Stage 7: LM Training: train_set=${data_feats}/lm_train_filtered_2000.txt, dev_set=${lm_dev_text}"
    ...
    if [ "${num_splits_lm}" -gt 1 ]; then
        ...
        if [ ! -f "${_split_dir}/.done" ]; then
            ...
            ${python} -m espnet2.bin.split_scps \
              --scps "${data_feats}/lm_train_filtered_2000.txt" "${lm_stats_dir}/train/text_shape.${lm_token_type}" \
        ...
        _opts+="--train_data_path_and_name_and_type ${_split_dir}/lm_train_filtered_2000.txt,text,text "
        _opts+="--train_shape_file ${_split_dir}/text_shape.${lm_token_type} "
        _opts+="--multiple_iterator true "

    else
        _opts+="--train_data_path_and_name_and_type ${data_feats}/lm_train_filtered_2000.txt,text,text "
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ] && ! [[ " ${skip_stages} " =~ [[:space:]]9[[:space:]] ]]; then
    log "Stage 9: Ngram Training: train_set=${data_feats}/lm_train_filtered_2000.txt"
    mkdir -p ${ngram_exp}
    cut -f 2- -d " " ${data_feats}/lm_train_filtered_2000.txt | lmplz -S "20%" --discount_fallback -o ${ngram_num} - >${ngram_exp}/${ngram_num}gram.arpa
